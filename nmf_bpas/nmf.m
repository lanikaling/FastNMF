...
%------------------------------------------------------------------------------------------------------------------------------
        ...
        HIS(1,9)=max(norm([gradW(gradW<0);gradH(gradH<0)]),norm([max(gradW(:),0).*W(:);max(gradH(:),0).*H(:)]));
...
%------------------------------------------------------------------------------------------------------------------------------
            ...
            HIS(idx,9)=max(norm([gradW(gradW<0);gradH(gradH<0)]),norm([max(gradW(:),0).*W(:);max(gradH(:),0).*H(:)]));
...
%             SC = getStopCriterion(ST_RULE,A,W,H,par.type,par.alpha,par.beta,gradW,gradH);
            SC = max(norm([gradW(gradW<0);gradH(gradH<0)]),norm([max(gradW(:),0).*W(:);max(gradH(:),0).*H(:)]));
