...
ITER_MAX=10000;      % maximum inner iteration number (Default)
...
HIS.prjg=init_delta;
HIS.prjg=max(norm([GradW(GradW<0);GradH(GradH<0)]),norm([max(GradW(:),0).*W(:);max(GradH(:),0).*H(:)]));
...
for iter=1:MaxIter,
    ...
    delta2= max(norm([GradW(GradW<0);GradH(GradH<0)]),norm([max(GradW(:),0).*W(:);max(GradH(:),0).*H(:)]));
    ...
    % Stopping condition
    if (delta2<=tol && iter >= MinIter) || HIS.cpus(end)>=MaxTime, 
